...
export default function KanbanBoard({ issues, repoUrl }: Props) {
  ...
  const handleDrop = (column: keyof typeof tasks) => {
    ...
    if (fromColumn === column) {
      ...
      const updatedFromColumn = tasks[fromColumn as keyof typeof tas].filter((t) => t !== task);
      const updatedToColumn = [...tasks[column]];
      const toIndex = draggedOverTask ? updatedToColumn.findIndex((t) => t.id === draggedOverTask.id) : updatedColumn.length
